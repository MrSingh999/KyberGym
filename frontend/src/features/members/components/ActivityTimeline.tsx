...
const activityConfig = {
  member_created: { icon: UserPlus, color: "text-primary", bg: "bg-primary/10" },
  ...
  profile_updated: { icon: FileText, color: "text-secondary", bg: "bg-surface-hover" },
  note_added: { icon: FileText, color: "text-warning", bg: "bg-warning/10" },
  workout_assigned: { icon: Dumbbell, color: "text-primary", bg: "bg-primary/10" },
            ...
            return (
              <div key={event.id} className="relative flex gap-4">
                {index !== activities.length - 1 && (
                  <div className="absolute left-4 top-9 bottom-[-24px] w-px bg-subtle" />
                )}
                <div className={cn(
                ...
                <div className="flex flex-col flex-1 pt-1 min-w-0">
                  <p className="text-sm text-primary leading-relaxed">{event.description}</p>
