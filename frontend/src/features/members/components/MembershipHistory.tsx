...
const statusConfig = {
  ...
  expired: { icon: StopCircle, label: "Expired", color: "text-muted", bg: "bg-surface-hover" },
      ...
      <WidgetBody
        ...
        emptyState={
          <WidgetEmptyState
            ...
            icon={<ScrollText className="h-6 w-6 text-muted" />}
          />
        }
        scrollable
      >
        <div className="space-y-3">
          {sorted.map((sub) => {
            ...
            return (
              <div
                key={sub.id}
                className="flex items-start gap-3 p-3 rounded-xl border border-default bg-surface-hover/50"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1 text-xs text-muted font-mono">
                  ...
                  {sub.notes && (
                    <p className="text-xs text-muted mt-1 italic">{sub.notes}</p>
                  )}
                </div>
              </div>
            );
