      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <h3 className="font-heading font-semibold text-base text-primary truncate">
                {plan.name}
              </h3>
              {plan.isPopular && (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-amber-600 dark:text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded-full">
                  <Star className="w-2.5 h-2.5 fill-current" />
                  Popular
                </span>
              )}
              {plan.isDefault && (
                <span className="text-[10px] font-bold uppercase tracking-wide text-primary/60 bg-surface-hover px-1.5 py-0.5 rounded-full border border-default">
                  Default
                </span>
              )}
            </div>
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <details className="group/menu">
              <summary className="list-none p-1.5 rounded-lg hover:bg-surface-hover text-muted hover:text-primary cursor-pointer transition-colors">
              ...
              <div className="absolute right-0 top-8 z-20 bg-surface border border-default rounded-xl shadow-lg py-1.5 w-40 text-sm">
                ...
                {plan.status !== 'archived' && (
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-surface-hover text-destructive transition-colors"
                  >
        ...
        <div className="mb-4">
          <div className="flex items-baseline gap-1.5">
            <span className="text-3xl font-bold font-heading text-primary tabular-nums">
              ${plan.price}
            </span>
          ...
          {plan.joiningFee && plan.joiningFee > 0 && (
            <p className="text-xs text-muted mt-0.5">+ ${plan.joiningFee} joining fee</p>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-subtle text-xs text-muted">
          ...
          <span
            className={cn(
              'font-medium',
              plan.featureCount > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted',
            )}
          >
