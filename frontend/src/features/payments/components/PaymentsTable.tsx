...
export function PaymentsTable({
  ...
  const columns: ColumnDef<PaymentListItem>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <span className="font-medium text-primary">{row.original.memberName}</span>
          <p className="text-xs text-muted mt-0.5">{row.original.memberCode}</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue }) => <span className="font-semibold text-primary tabular-nums">${getValue<number>()}</span>,
    },
    {
...
function SortHeader({ label, column }: { label: string; column: any }) {
  ...
  return (
    ...
    >
      {label}
      {sorted === 'asc' ? <ArrowUp className="w-3.5 h-3.5 text-primary" /> :
       sorted === 'desc' ? <ArrowDown className="w-3.5 h-3.5 text-primary" /> :
