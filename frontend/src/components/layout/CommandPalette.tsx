...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm sm:flex sm:items-start sm:justify-center sm:pt-[15vh]">
      <Command 
        className="w-full h-full sm:h-auto sm:max-h-[60vh] sm:max-w-2xl bg-canvas sm:bg-elevated sm:border sm:border-border-default sm:rounded-2xl shadow-elevated overflow-hidden animate-fade-slide-up flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center border-b border-border-default px-4 shrink-0 bg-surface">
          <Search className="w-5 h-5 text-text-muted mr-3" />
          <Command.Input 
            autoFocus
            placeholder="Type a command or search..." 
            className="flex-1 h-14 sm:h-14 bg-transparent outline-none text-text-primary placeholder:text-text-muted text-sm"
          />
          <button 
            className="sm:hidden text-text-muted p-2 active:scale-95 touch-target-icon"
        ...
        <Command.List className="flex-1 overflow-y-auto p-2">
          ...
          <Command.Group heading="Quick Actions" className="text-xs font-semibold text-text-muted mb-2 px-2">
            <Command.Item 
          ...
          <Command.Group heading="Navigation" className="text-xs font-semibold text-text-muted mb-2 px-2 mt-4">
            <Command.Item 
